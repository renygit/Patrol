...
public class AutoScrollBackLayout extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        scroll_gravity = attr.getInt(R.styleable.AutoScrollBackLayout_scroll_gravity, Gravity.BOTTOM | Gravity.RIGHT);
        showScrollDistance = attr.getDimensionPixelSize(R.styleable.AutoScrollBackLayout_scroll_distance, 500);
        arrowIcon = attr.getResourceId(R.styleable.AutoScrollBackLayout_auto_arrow_icon, R.mipmap.go_top);
        attr.recycle();
    ...
    private View findTargetScrollView(View view) {
        if (view != null) {
            if (view instanceof RecyclerView || view instanceof NestedScrollViewEx || view instanceof AbsListView) return view;
    ...
    public void bindScrollBack() {
        if (wrapView != null && scrollBackView != null && mShowScroll) {
            if (wrapView instanceof RecyclerView) {
                addScrollListenerForRecyclerView();
            } else if (wrapView instanceof NestedScrollViewEx){
                addScrollListenerForNestedScrollView();
            } else if(wrapView instanceof AbsListView){
    ...
    private void addScrollListenerForNestedScrollView() {
        ((NestedScrollViewEx) wrapView).setScrollExListener(new NestedScrollViewEx.ScrollExListener() {
            @Override
            public void onScrollStateChanged(NestedScrollView scrollViewEx, int newState) {
                if (myScrollLitener != null)
                    myScrollLitener.onScrollStateChanged(scrollViewEx, newState);
            }

            @Override
            public void onScrolled(NestedScrollView recyclerView, int dy) {
                if (myScrollLitener != null) myScrollLitener.onScroll(recyclerView, dy, 0, 0);
            }
        });
    }
    ...
    private int getScrollDistance(int dy) {
        ...
        if (wrapView instanceof AbsListView) {
            ...
            NestedScrollViewEx scrollViewEx = (NestedScrollViewEx) this.wrapView;
            return dy;
        }
        return dy;
    }
